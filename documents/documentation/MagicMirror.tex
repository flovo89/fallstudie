...
\newglossaryentry{ssh}{
	name=SSH,
	description={Secure Shell ist ein Netzwerkprotokoll für verschlüsselte Netzwerkkommunikation}
}
...
\\\\Verschiedene Vorschläge kursieren im Internet über den Aufbau eines Magic Mirrors. Richtig etabliert hat sich jedoch nur ein \Gls{opensource} Projekt. Durch Verwendung davon können auch weitere Forderungen an das Projekt, wie zum Beispiel Energie zu sparen, abgedeckt werden. Gezielte Erweiterungen des Projekts durch Verwendung bekannter Kommunikationsprotokolle wie \Gls{http} und \Gls{mqtt} ermöglichen zahlreiche Zusatzoptionen. Durch Interaktion mit einer Android Applikation kann die Bedienung des Magic Mirrors ausgelagert werden.  
\\\\Die Umsetzung beginnt mit dem Inbetriebnehmen der Hardware, wobei ein Raspberry Pi 3 verwendet wird, und dem Aufsetzen des genannten \Gls{opensource} Projekts. Nächster Schritt ist die Einbindung der Energiesparmassnahme mit einem externen Infrarot Sensor gesteuert von einem Controller-Board mit integriertem \Gls{wlan} Chip. Durch Verwendung von \Gls{node-red} kann das Verhalten dieser Komponente korrekt genutzt werden und alle gesendeten \Gls{mqtt} Nachrichten können verarbeitet werden. Danach kommt die Integration der Android Applikation um Notizen auf dem Magic Mirror zu verwalten. Die Kommunikation zwischen der Android Applikation und dem Magic Mirror findet über eine \Gls{rest-api} statt. Zuletzt wird die Machbarkeit des Google-Assistants auf dem Raspberry Pi des Magic Mirrors getestet, wo jedoch die aufgenommene Sprachqualität ungenügend ist.
\\\\Folgende Punkte werden somit schrittweise erläutert:
\begin{itemize}
	\item Inbetriebnahme \Gls{opensource} Projekt.
	\item Inbetriebnahme und Aufsetzen der Software für den externen ESP32 Controller und Interaktion mit \Gls{node-red}.
...
Mit dem verbauten Bildschirm ergeben sich beinah unbegrenzte Möglichkeiten um Informationen zu präsentieren und diese ansprechend darzustellen. Dadurch passt ein Magic Mirror mit passendem Design gut in einen Wohnbereich.
...
Um zu evaluieren, ob mit dem \Gls{opensource} Projekt MagicMirror\textsuperscript{2} weitergearbeitet werden kann oder ein komplett eigenständiger Ansatz gewählt werden muss, werden folgende Bewertungskriterien genauer analysiert:
\begin{itemize}
	\item Modularität und Erweiterbarkeit
	\item Einarbeitungszeit
	\item Hardwareeinsatz
\end{itemize}
Die folgenden drei Unterkapitel behandeln kurz diese Punkte jeweils für das \Gls{opensource} Projekt, sowie für die komplette Eigenentwicklung. Dabei werden Bewertungen angegeben zwischen 1-10, wobei 10 sehr gut ist.
...
Der Laptop im System wird verwendet um Konfigurationen des Magic Mirror zu ändern. Der Vorgang ist so, dass eine \Gls{ssh} Verbindung zum Raspberry Pi aufgebaut wird, wo die Konfiguration angepasst werden kann.
...
\begin{itemize}
	...
	\item Mehrere Notizgruppen können hinzugefügt werden. Hier gewünscht sind \textit{TODO} und \textit{Shopping}.
...
\vspace{0.2cm}
\\Die Notizen werden als \Gls{json} übertragen. Dabei werden \Gls{http}-Get Befehle für die Zugriffe \textit{GetCompleteNote} und \textit{DeleteAllNotes} verwendet und ein \Gls{http}-Post Befehl für den Zugriff \textit{AddCompleteNote} mit einem \Gls{json}-Objekt im Body. Das \Gls{json}-Objekt sieht wie folgt aus:
\begin{figure}[h]
	\centering
	\includegraphics[width=0.7\textwidth]{JsonExample.png}
	\caption{\Gls{json} Beispiel}
\end{figure}
\\Die einzelnen Notizen werden in einem Array abgelegt. Beim aufgeführten Beispiel werden Brot und Salat zur Gruppe \textit{Shopping} und die Aufgabe \textit{Wohnung putzen} zur Gruppe \textit{TODO} hinzugefügt.
\\
\\Die nötigen Erweiterungen an dem Modul belaufen sich auf ein einzelnes JavaScript-File, wo zusätzliche Behandlungen gemacht werden müssen, je nach dem welche Anfrage kommt. Dabei werden die bisherigen (wo die ganze Information in der \Gls{url} vorhanden ist) weiter unterstützt.
...
Mit dem Framework von Espressif \cite{ESP-IDF:2019:Online} ist eine sehr umfängliche Basis gegeben, um Funktionalitäten auf dem ESP32 Mikrocontroller zu programmieren. Neben Zugriffen auf die Peripherien des Controllers, sind verschiedene Protokolle im Framework integriert, FreeRTOS kann eingebunden werden, wie auch ein Monitoring Dienst, der als Ausgabekonsole verwendet werden kann. Als Übersicht das Bild:
\begin{figure}[h]
	\centering
...
\end{figure}
\\Mit diesen beiden Geräten kann evaluiert werden, inwiefern der Google Assistant sinnvoll verwendet werden kann im Magic Mirror. Punkte die untersucht werden während der Evaluation:
\begin{itemize}
	\item Mögliche Distanz um Sprachbefehle zu senden.
	\item Qualität der Musikausgabe.
\end{itemize}
...
\\Das Resultat dieses Tests war positiv. Die Qualität ist vergleichbar mit jeder anderen Verbindung zu diesem Bluetooth-Speaker.
...
\\kann eine Aufnahme von drei Sekunden gemacht werden und wird anschliessend gleich abgespielt (hier über den 3.5mm Audio Ausgang). 
...
\\Der wahrscheinlichste Grund für diese schlechte Qualität ist das günstige Mikrophon. Jedoch ist es nicht im Inhalt dieser Semesterarbeit weitere Untersuchungen in diesem Bereich zu tätigen.
